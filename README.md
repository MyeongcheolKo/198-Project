...
This build reads **Firestore** collection **`SensorData`**, where **each document** has arrays like `Heart Rate`, `SPO2`, `AcX`, `AcY`, `AcZ`, `Temp`. It flattens arrays to a live time series and renders a nurse-friendly UI.

## Configure

1. Copy `firebase-config.sample.js` → `firebase-config.js` and paste your web app config from Firebase Console.
2. If you want the big number to reflect a different channel, edit `SCORE_CHANNEL` in `app_firestore.js` (e.g., `'Heart Rate'`).
...
- Fields (arrays): `Heart Rate`, `SPO2`, `AcX`, `AcY`, `AcZ`, `Temp`
